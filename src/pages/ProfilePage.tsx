...
export function ProfilePage() {
  ...
  const userSessions = useMemo(() => {
    ...
    return sessions.filter(s => s.firebase_uid === currentUser.uid)
  }, [sessions, currentUser])

  useEffect(() => { document.title = 'Perfil · Zentro' }, [])
    ...
    <DashboardLayout userInitials={userInitials}>
      <div className="pf-root">
        ...
        <div className="pf-card pf-header-card">
          ...
          <div className="pf-header-card__right">
            ...
            <button className="pf-edit-btn" type="button" onClick={() => setEditing(true)}>
              Editar perfil
            </button>
          </div>
        </div>
        ...
        <section className="pf-section">
          <h2 className="pf-section__title">Mis sesiones creadas</h2>
            ...
            {userSessions.map((ses) => (
              <div key={ses.id} className="pf-session-card">
                ...
                <div>
                  ...
                  <p className="pf-session-card__date">{ses.date} · {ses.startTime} - {ses.endTime}</p>
                  {ses.location && (
                    <p className="pf-session-card__date">📍 {ses.location}</p>
                  )}
                </div>
              </div>
            ))}
            {userSessions.length === 0 && (
              <p className="pem-empty">No has creado sesiones todavía.</p>
            )}
          </div>
        </section>
      ...
      {editing && (
        <ProfileEditModal
          ...
          onSave={(updated) => { saveProfile(updated); setEditing(false) }}
        />
      )}
    </DashboardLayout>
  )
